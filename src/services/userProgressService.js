import User from "../models/User.js";

export const addCompletedLesson = async (userId, lessonId) => {
  const user = await User.findById(userId);
  if (!user) throw new Error("User not found");

  const alreadyCompleted = user.progress.includes(lessonId);
  if (alreadyCompleted) return user.progress;

  user.progress.push(lessonId);
  await user.save();
  return user.progress;
};

export const updateStreak = async (userId, streakData) => {
  const user = await User.findById(userId);
  if (!user) throw new Error("User not found");

  if (!streakData.streak || typeof streakData.streak.current !== "number") {
    throw new Error("Invalid streak data");
  }

  if (!user.streak) {
    user.streak = {};
  }

  user.streak.current = streakData.streak.current;
  user.streak.lastUpdate = streakData.streak.lastUpdate || new Date();

  await user.save();
  return user.streak;
};

export const updateAchievements = async (userId, achievements) => {
  const user = await User.findById(userId);
  if (!user) throw new Error("User not found");

  user.achievements = achievements;
  await user.save();
  return user.achievements;
};

export const addXP = async (userId, amount) => {
  const user = await User.findById(userId);
  if (!user) throw new Error("User not found");

  user.xp += amount;
  await user.save();

  return user.xp;
};
